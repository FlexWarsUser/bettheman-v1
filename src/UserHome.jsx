  ...
  try {
    ...
    const isNumeric = /^\d+$/.test(ql);
      ...
      list = list.filter(ev => {
        ...
        if (name.startsWith(ql) || name.includes(ql)) return true;
        const m = (ev.name || '').match(/\b(\d{1,2})[:.]?(\d{2})\b/);
        if (!m) return false;
        const h = parseInt(m[1], 10);
        const min = m[2];
        const forms = [String(h) + min, String(h + 12) + min, String(h - 12) + min];
        return forms.includes(ql);
      });
    } else {
